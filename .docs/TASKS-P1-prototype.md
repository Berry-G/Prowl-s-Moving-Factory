...
> ### 🔴 2026-08-29 추가 규칙 (결정 11) — 이미 구현했다면 확인할 것
> **보호대상 체력 표시는 숫자·정확한 % 를 노출하지 않는다. 구간(눈금 바)으로만 표시한다.**
> - 내부값은 **float**. 사망 판정은 `hp <= 0f` (`== 0` 금지), 데미지 적용 후 **0 미만 클램프**
> - UI 는 값이 바뀔 때가 아니라 **표시 구간이 바뀔 때만** 갱신 (매 프레임 갱신 금지)
> - 이유: HP 100 이면 % 는 사실상 정수 표시인데 실질 여유는 `100 / (마리 수 × DPS)` 라
>   **체감상 3칸짜리**다. 87% 로 보이면 "아직 여유 있네" 로 오독한다
> - **적 체력바는 그대로 둔다.** 이 규칙은 보호대상에만 적용된다
> → 상세: `GDD.md` §3
...
## `[x]` G-12. 일시정지 메뉴 (우상단) (2026-08-29 완료)
...
## `[x]` G-13. 디버그 오버레이 개편 — 접기 + 가독성 (2026-08-29 완료)
...
## `[x]` G-16. 고용 패널 정보 표시 (2026-08-29 완료)
...
## `[x]` G-17. 스폰 테이블 — 적 종류를 데이터로 (2026-08-29 완료)

> ### 🔴 2026-08-29 추가 (결정 13) — 진행도별 체력 열
> 모체와 보호대상 속도가 둘 다 고정이므로 **거리는 진행도의 함수**다.
> 거리로 인한 난이도 차이는 **스폰되는 적의 체력**으로 조절한다
> → 스폰 테이블에 **진행도별 체력 배율 열 하나**면 끝난다. 별도 시스템을 만들지 마라.
>
> **⚠️ 실시간 거리에 적 HP 가 반응하게 만들지 마라. 고무줄 난이도로 읽힌다.**
> 어디까지나 *진행도*(보호대상이 지나온 노드 등)의 함수여야 한다.
>
> **⚠️ 모체 속도를 난이도 변수로 쓰지 마라.** 모체에게 잡히는 일은 없다 → `GDD.md` §7
...
## `[x]` G-19. 스폰 리듬 — 묶음과 휴지 (2026-08-29 완료)
...
## `[x]` G-20. 버스트 모드 — **모체가 추적 에너지를 생산으로 전환** (2026-08-29 완료)
...
> ## 🔴 2026-08-29 재정의 (회의 결정 13) — 이전 설명은 틀렸다
>
> **컨셉:** 모체가 추적을 일시적으로 포기하고, **추적에 쓸 에너지를 생산에 몰빵**한다.
> 고장난 기계가 "화가 나서" 몰아치는 게 아니라 **자원 배분**이라 설명이 필요 없다.
>
> | | ~~이전 문서~~ | **지금** |
> |---|---|---|
> | 버스트 구간 | ~~정비 타이밍~~ | **최대 부하 구간** |
> | 정비 타이밍 | ~~버스트 중~~ | **버스트 직후 빈 구간** (모체가 뒤처져 적 도달까지 시간이 걸림) |
> | 예고 | ~~별도 UI 경고~~ | **모체가 멈추는 것 자체가 신호** |
>
> **리듬:**
> ```
...
2. **효과** — 모체 **이동 정지** + 스폰 밀도 급증
3. **⚠️ 총량 보존** — 버스트는 적 **총량을 늘리는 게 아니라 밀도만 바꾼다.**
   총량이 늘면 순수 페널티가 되어 플레이어가 할 수 있는 게 없다.
   보존이어야 **"버스트 전에 미리 배치를 몰아둔다"** 는 대비 플레이가 성립한다
4. **지속** — `_burstDuration` 후 `Chasing` 복귀
5. **빈 구간 상한** — 정지로 벌어진 거리가 그대로 빈 시간이 된다. 너무 길면 게임이 죽는다.
   → 추격 재개 시 모체 속도를 잠깐 올려 회수한다(에너지를 다시 추적에 몰빵 — 설정상 대칭)
6. **예고는 정지 그 자체.** 별도 경고 UI 를 만들지 마라. 모체 색 변경 정도만 보조로
7. 같은 트리거로 두 번 진입하지 않는다

**추가할 SO 필드:**
`_burstTriggerNodeIds`(string[]) · `_burstDuration`(**10.0** — 8~12초 시작점) ·
`_burstVolleyCount` · `_burstRestSeconds` (밀도만 조절, 총량 보존) ·
`_burstRecoverySpeedMultiplier`(1.6) · `_burstRecoverySeconds`(4.0)

**DoD:**
- 트리거 통과 시 모체가 멈추고 스폰 **밀도**가 급증한다
- **버스트 한 사이클의 적 총량이 평시 같은 시간 대비 크게 늘지 않는다** ← 총량 보존 확인
- 버스트가 끝나면 **눈에 띄는 빈 구간**이 생기고, 그동안 배치를 정비할 수 있다
- 빈 구간이 과도하게 길어지지 않는다 (추격 재개 시 속도 회수 동작)
- 모체가 멈추는 것만으로 "뭔가 온다" 가 읽힌다
- 같은 트리거로 재진입하지 않는다 / 일시정지·배속이 정상 동작한다

**⚠️ 함정**
- **트리거를 거리 비교로 하지 마라.** 노드 *통과* 이벤트로 잡아라. 거리로 하면 배속에서 건너뛴다.
- 노드 ID 오타가 런타임까지 간다. 시작 시 조회해 **없으면 `LogError`**.
- **버스트 중을 "안전한 배치 구간" 으로 만들지 마라.** ← 이전 문서의 오류다.
  버스트 중은 **가장 바쁜 구간**이고, 정비는 **그 다음 빈 구간**에서 한다.
- 도메인 리로드 OFF: 트리거 발동 여부를 `static` 에 담지 마라.
- **모체 속도를 난이도 변수로 쓰지 마라.** 모체에게 잡히는 일은 없다.
  거리로 인한 난이도 차이는 **스폰되는 적의 체력**으로 조절한다 (G-17 스폰 테이블에 진행도별 체력 열).
  **실시간 거리에 HP 가 반응하게 만들면 고무줄 난이도로 읽힌다.**
...
## `[◐]` G-21. 밸런스 재조정 + 시연 검증 패스 (2026-08-30 — **맵 변경으로 재측정 필요**, 눈·귀 항목도 미확인)
...
**문제:** 한 판 완주 41.7초 — 목표 90~150초의 절반도 안 됐다.
경로 총길이 50.0칸 ÷ 보호대상 속도 1.2 = 41.7초로 **완주 시간은 전적으로 보호대상 속도가 정한다.**
조정 우선순위 ①업그레이드 비용·②스폰 휴지는 완주 시간을 전혀 바꾸지 못하므로 ③보호대상 속도가 유일한 지렛대였다
(④맵 길이는 최후 수단이라 손대지 않았다).

**처방:** 이동 속도 전부에 같은 배율 ×0.35. 상대 역학(추격·따라잡기·접촉·버스트 빈 구간)은 그대로 두고
시계만 늘린다. 체감이 느리면 배속 버튼(1x/2x/4x)이 이미 있다.

| 대상 | 필드 | 전 | 후 | 이유 |
|---|---|---|---|---|
| Stage_Greybox | `_escorteeSpeed` | 1.2 | **0.42** | 완주 41.7 → 119.0초 (목표 90~150 중앙) |
| Stage_Greybox | `_motherSpeed` | 0.8 | **0.28** | 보호대상보다 느려야 한다는 제약 유지 (같은 비율) |
| Robot_Walker | `_moveSpeed` | 2.0 | **0.70** | 보호대상 대비 속도비 유지 |
| Robot_Scout | `_moveSpeed` | 3.4 | **1.19** | 〃 |
| Ally_CatFolk | `_moveSpeed` | 3.2 | **1.12** | 행군 시간이 판 길이에서 차지하는 비중 유지 |
| Ally_RatFolk | `_moveSpeed` | 1.8 | **0.63** | 〃 |

**손대지 않은 것:** 체력·공격력·간격·비용·환불률·쿨다운·자원 수급·지름길 가격.
수급과 스폰은 둘 다 초당이라 판이 길어지면 함께 늘어난다 — 서로의 비율은 보존된다.

### 계측 결과 (2026-08-30 — 맵 변경 후 재측정)

**⚠️ 계측 방법이 바뀌었다. 아래 표를 읽기 전에 이것부터 읽어라.**

이 판은 **확률적이다.** 같은 설정·같은 전략으로도 판마다 승패가 갈린다
(스폰 테이블 워커/스카우트 70:30 랜덤). **단일 표본으로 밸런스를 판정하지 마라.**
2026-08-29 표가 전부 1회 측정이었으므로 그 수치는 신뢰할 수 없다.

또한 자동 플레이 하네스에 **두 가지 함정**이 있었다. 둘 다 실제로 잘못된 결론을 만들었다:

| 함정 | 증상 | 잘못 내렸던 결론 |
|---|---|---|
| `RestartStage()` 직후 씬 리로드 완료를 안 기다림 | 직전 판의 **최종 상태를 그대로 반환** (동일 수치가 반복되면 그 신호다) | "혼합이 쥐 단독을 앞선다" |
| 보호대상 최근접 칸이 도달 불가면 **그 프레임 고용을 통째로 포기** | 돈이 남은 채 5기에서 멈춤 | "쥐 독주다 / 고양이는 못 쓴다" |

→ **하네스는 시작 상태를 검증해야 한다** (새 세션 · 아군 0 · 적 0 · hp 100 · t<3).

#### 계측 (원본 SO 값, 하네스 수정 후, 보호대상 추종 배치)

| 운용 | 표본 | 승 / 패 | 승리 시 | 패배 시 |
|---|---|---|---|---|
| 교대 혼합 | 6 | **4 / 2** | 119.05초 · hp **100** · 아군 15기 | 25~28초 · 아군 5기 |
| 고양이(근접)만 | 3 | **2 / 1** | 119.05초 · hp **100** · 아군 24기 | 26초 · 아군 7기 |
| 쥐(원거리)만 | 3 | **1 / 2** | 119.05초 · hp **100** · 아군 10기 | 28초 · 아군 3기 |
| 무조작 방치 | 1 | 0 / 1 | — | **27.25초 패배** |

| 항목 | 결과 | 판정 |
|---|---|---|
| 한 판 완주 | **119.05초** (승리 시 항상 동일 — 이동거리 고정이라 결정적) | ✅ 90~150초 |
| 승리·패배 재현 | 둘 다 여러 번 재현 | ✅ |
| 동시 생존 적 최댓값 | **6** (승리 판은 3) | ✅ 풀링 기준 40 밑 |
| 한 판 콘솔 | 게임 경고·에러 **0건** (아래 PathFollower 수정 후) | ✅ |
| SO 오염 | 여러 판 돌린 뒤에도 `.asset` 무변경 | ✅ |
| **원거리 독주** | **아니다.** 오히려 쥐 단독 승률이 가장 낮다 | ✅ |
| **근접 단독 운용** | **가능하다.** 다만 같은 승리에 아군이 2.4배 필요(24기 vs 10기) | ✅ |

#### 배치 칸 (정적 계측 — 결정적, 신뢰도 높음)

| 항목 | 값 |
|---|---|
| 배치 가능 칸 | **391** |
| 그중 **마을에서 실제로 도달 가능** | **213 (54%)** |
| 도달 불가 | **178** — 도로 건너편 구역. 마을이 없다 (HANDOFF §6-6 과 일치) |
| 고양이(사거리 1.6) 도로에 닿는 칸 | 102, 그중 도달 가능 **51 (전체의 24%)** |
| 쥐(사거리 6) 도로에 닿는 칸 | 374, 그중 도달 가능 **200 (전체의 94%)** |

> 2026-08-29 표의 "97칸 중 근접 36칸(37%)" 은 맵 변경 전 수치다. 분모가 213으로 바뀌었고
> **근접의 상대 지분은 37% → 24% 로 줄었다.** 그런데도 근접 단독 승률이 더 높은 이유는
> 비용당 DPS 가 2.6배이기 때문이다 (고양이 0.346 / 쥐 0.131).

### 🔴 미해결 — 이 단계에서 고칠 수 없다 (P-21 / 기획 판단으로 넘김)

**판이 첫 26~28초에 결정된다. 넘기면 남은 90초가 무의미하다.**

- 패배는 **전부** 25~28초에 일어난다. 그 관문을 넘긴 판은 **예외 없이 hp 100 무피해 승리**다.
  중간(아슬아슬한 승리)이 한 판도 나오지 않았다.
- 관문의 성패를 가르는 것이 **플레이어 판단이 아니라 스폰 랜덤**이다.
  동일 전략으로 승률 33~67%.
...
| 시도 | 결과 |
|---|---|
| 스폰 휴지 8.2 → 4.5 / 5.5 / 6.0 | 초반 관문만 더 가혹해지고 이분법은 그대로 |
| 고양이 공격 간격 0.45 → 0.25 | 하네스 오염 상태에서 낸 판단이었다. 수정 후 불필요해짐 |
| 시작 자원 150→260 · 초당 8→5 · 적 체력 커브 1.0→2.6 | 6/6 패배 |
| **적 체력 커브만 1.0 → 2.6** | 승리는 **여전히 무피해**. 후반 압박이 안 생긴다 |

> **왜 체력 커브로는 안 되는가 (계산):** 후반 아군 15기의 합계 DPS ≈ 225.
> 적 유입은 초당 0.32마리 × 체력 52(20×2.6) ≈ **16.6 HP/초**. **약 13~17배 과잉**이다.
> 체력을 2.6배가 아니라 10배로 올려도 못 따라간다.

> ### 📌 현장 결정 — 여기서 멈춘 이유
> 후반 압박을 만들려면 **진행도에 따라 스폰 밀도가 오르는 축**이 필요하다.
> 현재 `StageDefinition` 에는 그런 필드가 없다 (`_spawnRestSeconds` 는 상수).
> 새 SO 필드를 만드는 것은 **코드 변경**이고, 이 태스크는 *"SO 값만 만진다. 코드로 맞추지 마라"* 다.
> → **G-21 범위 밖.** `_enemyHealthByProgress` 는 **원본(전 구간 1.0)으로 되돌렸다.**
>   (G-17 이 기구만 만들고 값을 안 넣어 둔 상태였다 — 그것도 그대로 두었다.)

### 이번 패스에서 고친 코드 (밸런스 아님 — 버그)

| 파일 | 내용 |
|---|---|
| `Session/DeploymentController.cs` | `Update`/`UpdateRangeHover` 가 `GridSystem.Instance` 를 가드하지 않아 씬 파괴 순서에 따라 **매 프레임 NRE** (직전 세션 로그에 0.85초간 283건). `Update` 진입부에 조용한 가드 + `Start` 에 씬 누락 LogError |
| `Pathing/PathFollower.cs` | `SetRoute` 오탐 경고 **한 판 47건**. 허용치가 *나가는* 엣지 길이였는데, 재계산 시점의 액터는 *들어오는* 엣지 위에 있다. `route[0]` 에 붙은 엣지 중 **가장 긴 것**을 상한으로 바꿈 — 진단은 살고 오탐만 사라짐 |

### ⚠️ 아래 계측 결과는 **맵 변경 전(2026-08-29) 수치다 — 다시 재라**

2026-08-30 에 맵과 이동 규칙이 크게 바뀌었다 (배치 칸 130 → 391, 아군 우회 이동 도입).
난이도가 내려갔고 자동 플레이 기준 아군이 22기까지 깔린다. **밸런스를 처음부터 다시 잡아야 한다.**
자세한 내용은 `HANDOFF-2026-08-30.md` §2-①.

### 계측 결과 (2026-08-29 — 맵 변경 전)

| 항목 | 결과 |
|---|---|
| 한 판 완주 | **119.1초** (3회: 일반 / 근접만 / 원거리만 — 전부 동일, 이동거리 고정이라 결정적) |
| 무조작 방치 | **Defeat 26.1초** (진행 22%) — 방치하면 진다 |
| 승리·패배 재현 | 둘 다 재현 ✅ |
| 동시 생존 적 최댓값 | **3** — P-19(풀링) 기준 40 크게 밑돎. 풀링 불필요 |
| 근접(고양이) 유효 배치 칸 | 97칸 중 **36칸(37%)** 이 사거리 1.6 으로 도로에 닿는다 |
| 원거리(쥐) 유효 배치 칸 | **97칸(100%)** |
| 병종 단독 운용 | 근접만 승리 ✅ / 원거리만 승리 ✅ → **원거리 독주 아님** |
| 한 판 콘솔 | 게임 경고·에러 **0건** |

**⚠️ 미해결 관찰 (P-21 재미 판정으로 넘김 — 여기서 새 기능을 넣지 않는다)**
- 유능한 플레이는 **무피해 승리**(hp 100/100)다. 난이도 폭이 "방치=26초 패배 / 잘하면 무손실"로 양극단이다.
- 병종 단독으로도 이기므로 **두 병종을 섞을 이유가 아직 약하다.** G-01 의 역할 분담이 압박으로 이어지는지는 사람이 굴려 봐야 안다.
- 재배치 쿨다운 3초는 판이 2.9배 길어지면서 상대적으로 가벼워졌다. 값은 그대로 뒀다.

### 이 단계(G-12~G-20)에서 추가된 SO 필드

| SO | 필드 | 태스크 |
|---|---|---|
| StageDefinition | `_spawnTable` (SpawnEntry[]) | G-17 |
| StageDefinition | `_enemyHealthByProgress` (AnimationCurve) | G-17 / 회의 결정 13 |
| StageDefinition | `_spawnVolleyCount` · `_spawnVolleySpacing` · `_spawnRestSeconds` | G-19 |
| StageDefinition | `_burstTriggerNodeIds` · `_burstDuration` · `_burstVolleyCount` · `_burstRestSeconds` · `_burstRecoverySpeedMultiplier` · `_burstRecoverySeconds` | G-20 |
| StageDefinition | ~~`_motherSpawnEnemy`~~ 제거 (G-17), ~~`_motherSpawnInterval`~~ 제거 (G-19) | |
...
> ### ⚠️ 22번 판정 기준 — **그래픽은 판정 대상이 아니다** (2026-08-29 §3-6)
> 합격선이 "완성된 물건으로 보이는가" 인데 **그래픽은 후순위로 합의됐다**
> (AI 작업이라 매력적이지 않을 가능성을 이미 인식하고 있고, 크게 신경쓰지 않기로 했다).
>
> → **22번은 조작과 피드백만 본다.** 도형이 예쁜지, 색이 촌스러운지는 판정에서 제외한다.
> → 판정 질문은 이것이다: **"조작이 막히는 데 없이 5분을 굴릴 수 있는가.
>   무슨 일이 일어나는지 화면만 보고 읽히는가."**
> → 그래픽 지적이 나오면 **P2 아트 단계로 넘기고 여기서 붙잡지 마라.**
...
## `[x]` G-22. 근접 교전 이동 + 장애물 2종 (2026-09-01 완료)

> 사용자 지시로 추가된 태스크다. 근거는 **ADR-0017**.
> `GDD.md` §8 *"Deployed — 스스로 움직이지 않는다"* 를 **근접 병종에 한해** 완화한다.

### 무엇을 했나

1. **근접 병종(고양이)이 사거리 안의 적에게 걸어가 붙는다.**
   - 움직이는 범위는 **사거리 원 안**. 원의 중심은 **배치 슬롯에 고정**(`Attacker.SetRangeOrigin`)이라
     앞으로 나가도 **실효 사거리가 늘지 않는다** — G-21 계측치가 유효하게 남는 이유다.
   - 적이 사거리에서 빠지면 **슬롯으로 복귀**. 슬롯 예약은 계속 들고 있다.
   - **도로는 여전히 안 밟는다.** 적은 도로 위를 걸으므로 실제 그림은 *도로 경계까지 바짝 붙어서 때린다*.

2. **장애물이 2종이 됐다.** 통행은 똑같이 막고 **사거리 판정만 다르다**.

   | | 벽 `CellType.Blocked` | 물 `CellType.Water` |
   |---|---|---|
   | 통행 | 불가 | 불가 |
   | 근접이 너머를 때리기 | 불가 | **불가** |
   | 원거리가 너머를 쏘기 | 불가 | **가능** |

   **도로는 어느 쪽도 막지 않는다.** 막게 만들면 모든 적이 사거리 밖이 되어 게임이 성립하지 않는다.

3. **사거리 원을 2개 그린다** — 흐린 온전한 원(명목 사거리) + 진한 잘린 원(장애물에 깎인 실효 사거리).
   중심·반지름이 바뀔 때만 다시 잰다 (슬롯 고정이라 판 중에 변하지 않는다).

### 맵 변경

물 `(25,7)~(29,8)` 10칸 — **도로 S7(y=6, 탈출 직전) 바로 위**.
도로에서 먼 물은 의미가 없다(사거리가 안 닿아 병종 차이가 안 보인다).
동쪽 마을(27,9)에서 탈출 구간을 지킬 때 **쥐는 물 너머로 쏘고, 고양이는 x=30 으로 돌아 들어가야 한다.**

```
배치 가능 칸  391 → 381   (물 10칸이 빠졌다)
도로 51 · 벽 132 · 마을 2 — 그대로
```

**통행이 안 바뀌었으므로 경로·완주 시간은 그대로다.** 다만 동쪽 구간 배치 선택지가 10칸 줄었다.

### 건드리지 않은 것 (의도)

- **적(로봇)에게는 시야 판정을 켜지 않았다.** 켜면 밸런스가 바뀌어 G-21 을 다시 재야 한다.
- **원거리 병종은 움직이지 않는다.** GDD §8 은 원거리에 대해 그대로 유효하다.
- ADR-0003(블로킹 없음) 유지 — 붙어도 적을 멈추지 않는다. 적은 통과하면서 맞는다.

### 검증 (2026-09-01, Unity CLI)

```
recompile        completed / errors 0
console warning  0건 (파이프라인 자체 배너 제외)
run_tests        EditMode 37/37 Passed  (기존 32 + ObstacleSightTests 5)
```

플레이 모드 실측 (`EditorApplication.Step`):

| 확인 | 결과 |
|---|---|
| 물 너머 — 근접 시야 / 원거리 시야 | `False` / `True` |
| 벽 너머 — 근접 시야 / 원거리 시야 | `False` / `False` |
| 도로 너머 — 시야 / 통행 | `True` / `False` |
| 사거리 원 중심 = 슬롯 | 일치 |
| 교전 중 슬롯 이탈 최대 | `0.500` (사거리 1.60 이내, 도로 경계에서 정지) |
| 적이 없을 때 이탈 | `0.000` (슬롯 복귀) |

### 🔴 같이 잡은 버그 — 배치된 유닛이 이동 명령을 안 받던 문제

사용자 보고: *"쥐수인이 이동하지않아. 배치된 유닛이 모두 이동하지않아."*
**G-22 와 무관한 기존 버그**이고, 재컴파일 타이밍에 따라 나타났다 사라졌다 한다.

**원인 — 좌클릭 한 번을 두 컨트롤러가 각자 처리하는데 순서가 정해져 있지 않았다.**

`MonoImporter.GetExecutionOrder` 실측: `DeploymentController`·`SelectionController` **둘 다 0**
= Unity 내부 임의 순서. 둘 다 `wasPressedThisFrame` 을 읽으므로 같은 클릭이 양쪽에 들어간다.

| 실행 순서 | 결과 |
|---|---|
| Deployment 먼저 | `TryRedeploy` 실행 → **이동됨** |
| Selection 먼저 | 빈 칸이라 `Select(null)` → 선택 해제 → Deployment 가 `Selected == null` 을 봄 → **이동 안 됨** |

증상이 *"선택은 되는데 이동만 안 된다"* 인 이유가 이것이다.
`[이동]` 버튼(G-15)도 결국 맵 클릭이 필요하므로 같이 죽는다.

**고친 방법** — 기존 `CancelledThisFrame`(Esc 처리) 관례를 그대로 따랐다.

1. `DeploymentController` 에 `[DefaultExecutionOrder(-100)]` — **명령이 선택 갱신보다 먼저**라고 못박음
2. `RedeployedThisFrame` 추가 — 재배치가 **성공했을 때만** 세운다.
   `SelectionController` 가 그 프레임의 클릭으로 선택을 지우지 않는다
   (실패했을 땐 안 세우므로 "다른 유닛 클릭 = 그 유닛 선택" 은 그대로 동작한다)
3. `ShortcutController` 에도 같은 가드 — 순서가 확정되면서 마커 근처 배치 가능 칸을 클릭할 때
   *이동 명령 + 지름길 패널* 이 한 클릭에 같이 터질 수 있게 됐다

**G-22 가 만든 회귀도 하나 같이 고쳤다.** `SelectionController.FindAllyAt` 이 몸 위치의 셀로만
유닛을 찾고 있었다 — 근접 병종은 적에게 붙으러 슬롯을 벗어나므로 **자기 자리를 클릭해도 선택이 안 된다.**
배치된 유닛은 **슬롯으로 먼저** 찾도록 바꿨다 (슬롯은 예약되어 있어 판정이 유일하다).

검증 (플레이 모드 실측): 슬롯 클릭 선택 `True` / 몸셀 클릭 선택 `True` /
재배치 `marching=True` · `RedeployedThisFrame=True` · 선택유지 `True` / 이동거리 2.49 후 도착.

### 남은 것

- **사람 눈 확인**: 고양이가 붙어서 때리는 그림이 실제로 "근접"으로 읽히는가
- 물이 배치 판단을 실제로 만드는가 (동쪽 구간에서 병종을 갈라 쓰게 되는가) — P-21 재미 판정

---

## `[x]` G-23. 난이도 3종 (설정) + 처치 보상 (2026-09-02 완료)

> 사용자 지시. 근거는 **ADR-0018**. `GDD.md` §10 *"난이도 배율은 처치 보상에 걸지 않는다"* 를 정정했다.
...
적 체력·데미지·스폰량은 건드리지 않는다. **난이도는 경제 한 축만 움직인다.**

### 같이 구현된 것 — 처치 보상

회의 결정 4 로 채택돼 있었지만 **코드에는 없었다.** 지금까지 수입은 초당 8 고정 하나뿐이었다.
`Enemy.OnDied` 에서 지급한다. 보상이 4.5 처럼 정수가 아니라 `Wallet` 이 소수를 누적해
1 이 될 때마다 정수로 준다 — 시간 수입과 **같은 누적기**를 쓴다(따로 두면 잔돈이 버려진다).

### 조작 — 고르면 판이 다시 시작된다

설정 패널에서 언제든 고를 수 있고, 고르면 **확인 창 → 지금 판을 그 난이도로 재시작**한다.

처음에는 *판 중에는 잠그고 어둡게* 로 잡았다가 막다른 길을 발견했다 —
`PauseMenu` 는 판이 끝나면 중지 버튼을 숨기고 Esc 도 막으므로(G-12) **판 중에 잠그면 영영 못 바꾼다.**
"바꾼다 = 다시 한다" 로 묶으니 *판 중간에 수입 규칙이 바뀌어 밸런스를 못 읽는 문제*와
*불리할 때 쉬움으로 내려 위기만 넘기는 우회로*가 같이 사라졌다.

확인 창은 **무엇이 사라지는지**(배치·자원)와 **무엇이 달라지는지**(처치당·초당 수치)를 같이 보여준다.
경고에 숫자가 없으면 경고 구실을 못 한다.

### 🔴 밸런스 — 수입이 크게 줄었다

| | 이론 최대 수입 (전부 잡을 때) | 기존(초당 8) 대비 |
|---|---|---|
| 어려움 | 3 x 0.333 = **1.0/초** | 12% |
| 보통 | 4.5 x 0.333 = **1.5/초** | 19% |
| 쉬움 | 1.5 + 3 = **4.5/초** | 56% |

스폰 0.333마리/초 · 고양이 45G · 쥐 110G · 시작 150G 기준.
119초 완주 기준으로 보통은 총 **약 330G**(기존 약 1100G) — 배치 가능 수가 22기에서 7기 수준으로 떨어진다.

→ **G-21 밸런스는 난이도별로 다시 재야 한다.**
→ 어려움은 초반에 유닛을 못 사서 못 잡고, 못 잡아서 또 못 사는 **하강 나선**이 가능하다.
   시작 자원 150 이 유일한 안전판이다. 실플레이로 확인할 것.

### 검증 (2026-09-02, Unity CLI)

```
recompile        completed / errors 0
run_tests        EditMode 37/37 Passed
```
...
| 확인 | 결과 |
|---|---|
| 보통 — 시간만 6초 흐름 | `+0G` (시간 수입 없음) |
| 보통 — 적 2기 격파 | `+9G` (4.5 x 2, 소수 이월 정상) |
| 난이도 변경 → 재시작 | `Hard` / 처치보상 3 / 잔액 150 / t=1.2 / 아군 0 / 새 세션 |
| 쉬움 — 8초 경과 | `+24G` (초당 3) |

### 남은 것

- **사람 눈 확인**: 확인 창의 경고가 충분한가, 실수로 판을 날리지 않는가
- 난이도별 실플레이 밸런스 (위 하강 나선 포함) — P-21
