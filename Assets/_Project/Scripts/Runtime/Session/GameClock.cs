...
namespace PMF.Session
{
    ...
    public sealed class GameClock : MonoBehaviour
    {
        ...
        /// <summary>배치 UI(고용 패널 등)가 떠 있는 동안 걸리는 슬로우모션 배속.</summary>
        private const float UiSlowScale = 0.1f;
        ...
        {
            _paused = false;
            Time.timeScale = _uiSlowActive ? UiSlowScale : _speed;
        }

        /// <summary>배치 UI 가 뜨는 동안 정밀 조작을 위해 슬로우모션(0.1x)을 건다. 일시정지 중에는 아무 효과 없음.</summary>
        public void EnterUiSlowMotion()
        {
            if (_uiSlowActive) return;
            _uiSlowActive = true;
            if (!_paused) Time.timeScale = UiSlowScale;
        }
